...
script = [
        ...
        script: function (varibles) {
            ...
            timings.forEach((val, idx) => {
                if (val.distance < now ? (now - val.distance) / windowWidth <= 2 : (val.distance - now) / windowWidth <= 2) {
                    ...
                    line(RelX(-0.7 + myDis), RelY(0.6), RelX(-0.7 + myDis), RelY(0.6 - (val.bpm / 500) * 1.6));
                    if (idx == 0 ? true : timings[idx - 1].bpm != val.bpm) {
                        noStroke();
                        fill(info.uiColor);
                        textSize(Abs(0.04));
                        textAlign(LEFT, CENTER);
                        text(`${val.bpm}`, RelX(-0.7 + myDis + 0.009), RelY(0.6 - (val.bpm / 500) * 1.6 - 0.05));

                        strokeWeight(Abs(0.004));
                        stroke(info.tertiaryColor);
                        line(RelX(-0.7 + myDis), RelY(0.6 - ((idx == 0 ? val.bpm : timings[idx - 1].bpm) / 500) * 1.6), RelX(-0.7 + myDis), RelY(0.6 - (val.bpm / 500) * 1.6));
                    }
                    if (idx != timings.length - 1) {
                        strokeWeight(Abs(0.004));
                        stroke(info.tertiaryColor);
                        line(RelX(-0.7 + myDis), RelY(0.6 - (val.bpm / 500) * 1.6), RelX(-0.7 + (timings[idx + 1].distance - now) / windowWidth), RelY(0.6 - (val.bpm / 500) * 1.6));
                    }
                }
            });
            ...
            strokeWeight(Abs(0.004));
            stroke(info.tertiaryColor);
            line(RelX(-0.7), RelY(0.6 - (nowBpm / 500) * 1.6), RelX(-0.85), RelY(0.6 - (nowBpm / 500) * 1.6));

            noStroke();
            fill(info.tertiaryColor);
            textSize(Abs(0.04));
            textAlign(LEFT, CENTER);
            text(`${nowBpm}`, RelX(-0.85), RelY(0.6 - (nowBpm / 500) * 1.6 - 0.05));
            ...
            for (let i = 1; i <= 9; i++) {
                strokeWeight(Abs(0.002));
                stroke(info.uiColor);
                line(RelX(-0.7), RelY(0.6 - (i * 50 / 500) * 1.6), RelX(-0.68), RelY(0.6 - (i * 50 / 500) * 1.6));
    ...
    },
    {
        ...
        script: function (varibles) {
            ...
            strokeWeight(Abs(0.004));
            fill(info.secondaryUiColor);
            circle(RelX(lerp(0.4, 0.975, varibles.var["nowBeat"] / 1606)), RelY(-0.53), Abs(0.02));
